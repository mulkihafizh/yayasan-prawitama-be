...
exports.validateEmployeeData = (req, res, next) => {
    ...
    if (!email || !password || !name || !id_pegawai || !reg_number || !certi_number || !nik || !birth_date || !birth_place || !age || !duty_start_date || !sk_number || !graduation_date || !blood_type || !department || !employment_type || !education_history || !work_history || !address || !phone_number || !is_married) {
      return res.status(400).json({ message: 'Harus diisi' });
    }
  
    if (is_married) {
      if (!partner || !partner.name || !partner.birth_date || !partner.job) {
        return res.status(400).json({ message: 'Data partner harus diisi lengkap' });
      }
  ...
  exports.checkEmployeeAdmin = (req, res, next) => {
      if (req.headers,req.headers.authentication) {
        const user = User.findById(req.headers.authentication)
  ...
  exports.checkPayrollAdmin = (req, res, next) => {
      if (req.headers,req.headers.authentication) {
        const user = User.findById(req.headers.authentication)
